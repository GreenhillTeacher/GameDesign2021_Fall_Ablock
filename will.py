...
def display_level1():
    ...
    while check:
        ...
        print(keys)
        if mouse_pos[0] >= 355 and mouse_pos[0] <= 465 and mouse_pos[1]>=750 and mouse_pos[1]<=795:
        ...
        if keys[py.K_RIGHT] and x < 800-vel-WIDTH:  
            x += vel
            left = False
            right = True
        
        if keys[py.K_DOWN] and y >vel:
            y += vel
        ...
        if eve.type==py.MOUSEBUTTONDOWN:
            ...
            if currentDisplay==DISPLAY_MAIN_MENU:
                ...
                if mouse_pos[0]>=70 and mouse_pos[0]<=95 and mouse_pos[1]>=270 and mouse_pos[1]<=300: #71, 193. 93,193. 93, 212. 71, 211
                    ...
                    # walkRight = [py.image.load('FinalGame\Right1.png'),py.image.load('FinalGame\Right2.png'), py.image.load('FinalGame\Right3.png')]
                    # walkLeft=[py.image.load('FinalGame\Left1.png'),py.image.load('FinalGame\Left2.png'), py.image.load('FinalGame\Left3.png')]
                    # char=[py.image.load('FinalGame\standing.png')]
                    # bg=[py.image.load('gameimages/grassbkgimage2.jpg')]
                    # x=200
                    # y=200
                    # width=40
            ...
            #BACK BUTTON CODE for Main MEnu:
                ...
                elif currentDisplay==DISPLAY_SETTINGS_BACKGROUND_COLOR and mouse_pos[0]>=70 and mouse_pos[0]<=95 and mouse_pos[1]>=250 and mouse_pos[1]<=300:
                    bkgcolor=BLUE
                    window.fill(bkgcolor)
                    display_Title("Background Color", 70)
                    display_Menu(bkgcolors)
